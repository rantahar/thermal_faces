        ...
        for box in new_boxes:
            ...
            for i, old_box in enumerate(boxes):
                ...
                if box_iou(box, old_box) > max_overlap:
                    ...
                    boxes[i]["score"] = box[4]
                    boxes[i]["frame"] = frame_index
                    found_box = True
                    break
            if not found_box:
                # new box
                boxes.append({"x": box[0], "y": box[1], "width": box[2], "height": box[3], "score": box[4], "index": next_box_index, "frame": frame_index})
                next_box_index += 1

        if len(boxes) > 0:
            print(boxes)
            append_boxes_to_csv(boxes, output_file)
